...
ILOSOLVECALLBACK1(bpsolve, cplex_vertex_mapping, M) {
  getEnv().out() << " ---------- Hello World ----------- " << endl;
  getEnv().out() << "teste ------------------- " << endl;
}

ILOBRANCHCALLBACK1(bpbranch, cplex_vertex_mapping, M) {
  if ( getBranchType() != BranchOnVariable ) {
    cout << "another type of branch" << endl;
    return;
  }
...
bool vne::bbcgu(Graph& substrate, const Graph& virtual1, const Parameters param,
    Mapping& out_mapping) {
  IloEnv env;
  try {
    IloModel model(env);
    ...
    IloCplex cplex(env);
    cplex.use(bpsolve(env, M));
    cplex.use(bpbranch(env, M));
    cplex.setParam(IloCplex::MIPSearch, IloCplex::Traditional);
    // cplex.setParam(IloCplex::PreInd, 0);
    cplex.extract(model);
    cplex.solve();

    vne::cplexResult2Mapping(substrate, virtual1, cplex, M, D, out_mapping);
